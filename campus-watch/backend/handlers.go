...
func (h *Handlers) CreateSightingHandler(w http.ResponseWriter, r *http.Request) {
	...
	if userID != "" {
		...
		allowed, err := h.redis.AtomicCheckAndRecordRateLimit(ctx, key, 5, 24*60)
		if err != nil {
		...
		key := fmt.Sprintf("rate_limit:ip:%s", getClientIP(r))
		allowed, err := h.redis.AtomicCheckAndRecordRateLimit(ctx, key, 1, h.rateLimitMinutes)
		if err != nil {
			log.Printf("Error in atomic rate limit: %v", err)
		} else if !allowed {
